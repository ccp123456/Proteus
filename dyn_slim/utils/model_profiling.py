...
def module_profiling(self, input, output, verbose, use_cuda=True):
    ...
    if isinstance(self, (nn.Conv1d, nn.ConvTranspose1d)):
        ...
        self.n_seconds = 0
        ignore_zeros_t = [
            nn.BatchNorm1d, nn.Dropout1d, nn.Dropout,
            nn.Sequential, nn.ModuleList,
            nn.ReLU6, nn.ReLU, nn.MaxPool1d, Swish,
            nn.modules.padding.ZeroPad1d, nn.modules.activation.Sigmoid,
            nn.GroupNorm
        ]
...
def _count_sequential(m, verbose):
    ...
    for seq_m in m:
        if getattr(seq_m, 'n_macs', None) is None and verbose \
                and type(m) not in [nn.Sequential, nn.ModuleList, nn.BatchNorm1d]:
...
def model_profiling(model, height, batch=1, channel=3, use_cuda=True,
                    verbose=True):
    ...
    """
    model.eval()
    data = torch.rand(batch, channel, height)
    ...
    if verbose:
        ...
        for n, m in model.named_modules():
            if getattr(m, 'n_macs', None) is None and verbose \
                    and not isinstance(m, (nn.Sequential, nn.ModuleList, nn.BatchNorm1, Swish, nn.ReLU)):
