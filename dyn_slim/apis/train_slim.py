...
def train_epoch_slim(
    ...
    for batch in tqdm(loader, mininterval=2, desc='  - (Training)   ', leave=False):
        ...
        for sample_idx, model_mode in enumerate(sample_list):
            ...
            if model_mode == 'largest':
                ...
                if args.inplace_bootstrap:
                    loss = distill_loss_fn(output, F.softmax(output_largest[0], dim=1))
                    if args.ensemble_ib:
                        with torch.no_grad():
                            if hasattr(model_ema.ema, 'module'):
                                model_ema.ema.module.set_mode(model_mode)
        ...
        batch_time_m.update(endd - start)
    ...
    return acc_m.avg, losses_m_smallest.avg, losses_m_largest.avg, lr, OrderedDict([('loss_largest', losses_m_largest.avg),
...
def validate_slim(model, loader, loss_fn, args, log_suffix='', model_mode='largest'):
    ...
    with torch.no_grad():
        for batch in tqdm(loader, mininterval=2, desc='  - (Testing)   ', leave=False):
            ...
            prec_m.update(prec.item(), output.size(0))
        ...
        metrics = OrderedDict(
            [('prec', sum(total_acc) / len(total_acc)), ('loss', losses_m.avg), ('flops', flops_m.avg)])
    return sum(total_acc) / len(total_acc), total_pred, sum(total_time) / len(total_time), losses_m.avg, flops_m.avg, metrics
