...
from dyn_slim_1.models.dyn_slim_ops import DSpwConv2d, DSdwConv2d, DSBatchNorm2d, DSAvgPool2d, DSAdaptiveAvgPool2d
...
class MultiHeadGate(nn.Module):
    ...
    def forward(self, x):
        x_pool = self.avg_pool(x)  # pool
        x_reduced = self.conv_reduce(x_pool)  # pointwise conv 
        x_reduced = self.act1(x_reduced)  # activate
        attn = self.conv_expand(x_reduced) # pointwise conv 
        if self.attn_act_fn == 'tanh':
            attn = (1 + attn.tanh())
        else:
            attn = self.attn_act_fn(attn)  # sigmoid
        x = x * attn

        if self.mode == 'dynamic' and self.has_gate:
            channel_choice = self.gate(x_reduced).squeeze(-1).squeeze(-1)  # pointwise
            self.keep_gate, self.print_gate, self.print_idx = gumbel_softmax(channel_choice, dim=1,
